...
/**
 * The fix for "getting anywhere takes too many clicks". Every section and every
 * create action is one keystroke away regardless of where you are.
 */
export default function CommandPalette({
  ...
  // showing a stale search from ten minutes ago. Derived during render by
  // comparing against the previous value rather than via a setState-in-effect
  // — same pattern StudioShell uses to close the mobile drawer, and what
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="flex items-center gap-2 border-b border-admin-border px-3">
            <Search className="h-4 w-4 shrink-0 text-admin-muted" aria-hidden="true" />
            <input
              ...
              return (
                <li key={command.id}>
                  ...
                  <button
                    ...
                    onClick={() => run(command)}
                    className={cn(
                      "flex w-full items-center gap-2 rounded-admin-sm px-2 py-1.5 text-left text-[13px]",
                      FOCUS_RING,
                      active
                        ? "bg-admin-surface-hover text-admin-fg"
                        : "text-admin-fg"
                    )}
                  >
                    {command.create ? (
                      <Plus className="h-3.5 w-3.5 shrink-0 text-admin-muted" aria-hidden="true" />
                    ) : (
                      <span
                        aria-hidden="true"
