...
/**
 * The one table in the studio. Replaces the hand-written tables that each
 * re-implemented head/row/cell markup against the ADMIN_TABLE_* strings —
 * callers now supply column definitions and nothing else.
 *
 * TanStack stays headless: it supplies sort state and row order only.
 */
export default function DataTable<T extends RowData>({
  ...
  data: T[];
  // Each caller's columns array mixes several per-column TValue types (string,
  // boolean, computed unions). TanStack's own columnHelper.columns() return
  // type uses this same `any` for that reason — a narrower type here would
  // lose per-column inference rather than gain safety.
