      ...
      <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-5">
        {cards.map((card) => (
          <Link
            ...
            href={card.href}
            className={cn(
              "rounded-admin-lg border border-admin-border bg-admin-bg p-4 transition-colors hover:border-admin-border-strong hover:bg-admin-surface",
              FOCUS_RING
            )}
          >
            {/* A failed count renders as an em dash rather than 0 — silently
                showing zero would read as "you have no events". */}
